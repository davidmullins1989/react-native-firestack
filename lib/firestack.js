...
export class Firestack extends Singleton {
   ...
   * remote config
   */
  // Lazily create a single RemoteConfig instance and cache it on a private field to
  // avoid recursive getter calls and repeated allocations.
  get remoteConfig() {
    if (!this._remoteConfigInstance) {
      this._remoteConfigInstance = new RemoteConfig(this._remoteConfig);
    }
    return this._remoteConfigInstance;
  }
